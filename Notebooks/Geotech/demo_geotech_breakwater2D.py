...
m.serialize(
    Path(f"results/ScratchSettlementModel_ConfigNr-{config_idx}_Variant-{variant}.sli")
)
...
model.serialize(
    Path(f"results/ScratchStabilityModel_ConfigNr-{config_idx}_Variant-{variant}.stix")
)
# model.execute()
...
# Create soillayers
soil_layer_phased = SoilLayers.from_base_input_geom(
    soil_layer_phased.unary_union, materials.name.loc["Zand_Dijk_Za"], "20"
)
...
new_load_points = soil_layers._skip_consecutive_duplicates(
