...
class RockGrading:
    ...
    def __init__(self, grading=None, rho=2650, y_NLL=0.06, y_NUL=0.9):
        ...
        if grading is not None:
            ...
            msg = (
                "Invalid custom rock grading. Rock grading must be "
            ...
            grading = {
                "LMA_5/40": {"M50": [10, 20], "NLL": 5, "NUL": 40},
                "LMA_10/60": {"M50": [20, 35], "NLL": 10, "NUL": 60},
                "LMA_40/200": {"M50": [80, 120], "NLL": 40, "NUL": 200},
                #"LMA_15/300": {"M50": [45, 135], "NLL": 15, "NUL": 300}, commented out because mostly not used in projects
                ...
                "HMA_6000/10000": {"M50": [7500, 8500], "NLL": 6000, "NUL": 10000},
                "HMA_10000/15000": {"M50": [12000, 13000], "NLL": 10000, "NUL": 15000},
            }
...
class ConcreteArmour:
    ...
    def get_class(self, d):
        ...
        if class_volume == None:
            ...
            msg = (
                "given dn is out of range for the specified armour units"
...
class Xbloc(ConcreteArmour):
    ...
    def correction_factor(
        ...
        **kwargs,
            ...
            if np.arctan(1 / 20) <= slope_foreshore:
                if np.arctan(1 / 15) <= slope_foreshore:
                    if np.arctan(1 / 10) <= slope_foreshore:
                        ...
                        log.append(
                            "Xbloc: steepness of foreshore is greater " "than 1:10"
                        )
                    else:
                        correction.append(1.5)  # between 1:15-1:10
                        log.append(
                            "Xbloc: steepness of foreshore is between " "1:15 and 1:10"
                        )
                else:
                    correction.append(1.25)  # between 1:20-1:15
                    log.append(
                        "Xbloc: steepness of foreshore is between " "1:20 and 1:15"
                    )
            else:
                correction.append(1.1)  # between 1:30-1:20
                log.append("Xbloc: steepness of foreshore is between 1:30 and" " 1:20")
        ...
        msg = (
            f"Xbloc: used correction factor: {max_correction}, because "
...
class XblocPlus(ConcreteArmour):
    ...
    def __init__(self, rho=2400):
        ...
        units = {
            0.75: {"L1": 0.75, "L2": 1.51, "L3": 1.91, "h": 1.2, "Vc": 0.48},
    ...
    def correction_factor(
            ...
            if np.arctan(1 / 20) <= slope_foreshore:
                if np.arctan(1 / 15) <= slope_foreshore:
                    if np.arctan(1 / 10) <= slope_foreshore:
                        correction.append(2)  # steeper than 1:10
                        log.append(
                            "XblocPlus: steepness of foreshore is " "greater than 1:10"
                        )
                    else:
                        correction.append(1.5)  # between 1:15-1:10
                        log.append(
                            "XblocPlus: steepness of foreshore is "
                            "between 1:15 and 1:10"
                        )
                else:
                    correction.append(1.25)  # between 1:20-1:15
                    log.append(
                        "XblocPlus: steepness of foreshore is between " "1:20 and 1:15"
                    )
            else:
                correction.append(1.1)  # between 1:30-1:20
                log.append(
                    "XblocPlus: steepness of foreshore is between 1:30" " and 1:20"
                )
        ...
        max_index = correction.index(max_correction)
        msg = (
            f"XblocPlus: used correction factor: {max_correction}, because "
            f"{log[max_index]} was the maximum value"
        )
